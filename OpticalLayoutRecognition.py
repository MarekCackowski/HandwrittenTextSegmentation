...
class OpticalLayoutRecognition:
    ...
    @staticmethod
    def delete_duplicates(segments_in_lines, binary_histogram):
        y_initial = np.where(np.diff(binary_histogram) == 1)[0]
        y_final = np.where(np.diff(binary_histogram) == -1)[0]
        for i in range(len(y_initial) - 1):
            ...
            for first, segment1 in enumerate(current_line):
                for second, segment2 in enumerate(next_line):
                    if segment1 == segment2:
                        current_pixels = min(current_end_index, segment1[1] + segment1[3]) - max(current_start_index, segment1[1])
                        next_pixels = min(next_end_index, segment2[1] + segment2[3]) - max(next_start_index, segment2[1])
    ...
    @staticmethod
    def group_in_words(segments_in_lines, distance):
        words = []
        for segments_in_line in segments_in_lines:
            ...
            for i in range(len(segments_in_line) - 1):
                ...
                if x_max + distance > next_segment[0]:
    ...
    def make_image_with_segments(image, segments_in_lines):
        ...
        for segments_in_line in segments_in_lines:
            for segment in segments_in_line:
                x, y, w, h = segment
                image_with_segments = cv.rectangle(image_with_segments, (x, y), (x + w, y + h), (0, 255, 0), 1)
        return image_with_segments

    @staticmethod
    def make_image_with_words(image, words):
        image_with_words = image.copy()
        for word in words:
            ...
            image_with_words = cv.rectangle(image_with_words, (x_min, y_min), (x_max, y_max), (0, 255, 0), 1)
        return image_with_words

    def word_segmentation(self, image, binary_histogram):
        segments = self.prepare_segments(image)
        ...
        words = self.group_in_words(segments_in_lines, 20)
    ...
    # Jeszcze trzeba usunąć segmenty, które wychodzą za kwadrat słowa
    # Wyznaczam dolną i górną granicę słowa bez skeletonizacji dla każdego segmentu (nie wyrazu)
    # Segmenty nie mają ze sobą punktów wspólnych więc wiem, że w segmencie jest przynajmniej litera (poza bardzo małymi, które trzeba wykluczyć)
    # Wyznaczam medianę z górnego i dolnego konturu i traktuje wszystko między nią jako środkowa linia tekstu
    # Dla liter, które wystają znacznie poza środkową linię określam, gdzie się kończą na podstawie tego, co jest poza środkową linią
    # Na podstawie peaków w dolnej granicy ustalam, gdzie dzielić słowa
    # Peaki nie mogą zaczynać się od pionowego (zakładam, że jak ktoś pisze 'l' jako linię to nie łączy go z kolejną literą)
    # Peaki nie mogą być praktycznie poziome (H)
    # Zakładam, że 'i' ma kropkę nad sobą i na tej podstawie rozdzielam litery typu 'u' i 'w' (2 szczyty to 'u', 3 'w')
    ...
    @staticmethod
    def calculate_top_line(top_contour, bottom_contour, height):
    ...
    # TODO test 3
    def merge_capital_letter(self, top_peaks, bottom_peaks, segment_image):
        ...
        top_line = int(self.calculate_top_line(top_contour, bottom_contour, segment_image.shape[0]))
        # print('top_line', top_line)
        is_capital = False
        to_delete = []
        if segment_image.shape[0] > 50:
            for i in range(min(25, segment_image.shape[1] - 1)):
                if (top_contour[i] < top_line - 5 or top_contour[i] < 10) and top_contour[i] != 0:
                  is_capital = True
        if is_capital:
            for i in range(1, len(new_bottom_peaks) - 1):
                if new_bottom_peaks[i] < 40 and len(np.where(skeleton[:10, new_bottom_peaks[i] - 5:new_bottom_peaks[i + 1]] == 0)[0]) + len(np.where(skeleton[:top_line - 5, new_bottom_peaks[i] - 5:new_bottom_peaks[i + 1]] == 0)[0]) >= 10:
                    # print(new_bottom_peaks[i])
                    for j in range(new_bottom_peaks[i], new_bottom_peaks[i + 1] - 2):
                        if top_contour[j + 1] - top_contour[j] >= 10:
                            is_capital = False
                    letter_parts = self.prepare_letter_parts(segment_image, new_bottom_peaks, 0, i + 1, True)
                    if (is_capital and np.all(top_contour[new_bottom_peaks[i] - 3:new_bottom_peaks[i + 1]] != 0)) or (len(letter_parts) < 2 and new_bottom_peaks[i + 1] < 60):
                        to_delete.append(i)
    ...
    def calculate_diff_counts(skeleton, bottom_contour, top_contour, first_top_peak, middle_top_peak, last_top_peak, top_value):
        ...
        for i in range(max(first_top_peak - 5, 0), middle_top_peak):
            if top_contour[i] != top_value and bottom_contour[i] - top_contour[i] > 5:
    ...
    def prepare_letter_parts(self, word_image, peaks, begin, end, is_whole):
        ...
        letter_parts = self.prepare_segments(bordered_image)
        return letter_parts

    @staticmethod
    def assign_new_peaks(new_bottom_peaks, first_top_peak, top_value, bottom_value, first_bottom_peak, last_bottom_peak, last_bottom):
        if last_bottom != 0:
        ...
        if first_top_peak not in new_bottom_peaks:
            new_bottom_peaks = np.concatenate((new_bottom_peaks, [max(0, first_top_peak - 5)]))
        new_bottom_peaks = np.sort(new_bottom_peaks)
        return new_bottom_peaks

    def repair_oversegmentation_of_w_or_m(self, top_peaks, bottom_peaks, segment_image, is_top_first):
                ...
                if len(first_bottom_peaks) == 1 and len(last_bottom_peaks) == 1 and top_contour[middle_top_peak] != top_value:
                    ...
                    if first_bottom != bottom_value and last_bottom != bottom_value:
                        ...
                        if first_bottom < middle_leg and last_bottom < middle_leg:
                            first_top = top_contour[first_top_peak]
                            print('first_top', first_top)
                            if (top_contour[first_top_peak + 3] != top_value or first_top_peak == 0) and first_bottom + 7 < left_leg and first_bottom + 3 < right_leg:
                                last_top = top_contour[last_top_peak]
                                print('last_top', last_top)
                                if (top_contour[last_top_peak - 3] != top_value or last_top_peak == segment_width) and last_bottom + 7 < right_leg and last_bottom + 3 < left_leg:
                                    ...
                                    if last_top_peak > middle_top_peak > first_top_peak and top_value < first_bottom - 1 and top_value < last_bottom - 1:
                                        ...
                                        if diff_count_right <= 5 and diff_count_left <= 5:
                                            if sum(first_histogram) // 255 < 5 and sum(second_histogram) // 255 < 5:
                                                print(left_leg, right_leg, last_top, right_leg, first_bottom, last_bottom, diff_count_right, diff_count_left)
                                                ...
                                                if len(letter_parts) < 2:
                                                    ...
                                                    to_delete_bottom.append([first_top_peak, top_value, bottom_value, first_bottom_peaks[0], last_bottom_peaks[0], bottom_contour[last_bottom_peaks[0]]])
                                                    i += 2
            i += 1
    ...
    def adjust_letter_e_or_c(self, word_image, peaks):
        ...
        for i in range(len(adjusted_peaks)):
            ...
            if column > 0 and 10 <= self.count_white_pixels(skeleton, column, bottom_contour, top_contour) <= 25:
                column -= 1
    ...
    def repair_oversegmentation_of_n(self, bottom_peaks, top_peaks, width, segment_image):
        skeleton = self.skeletonize(segment_image)
        ...
        for bottom_peak in bottom_peaks:
            ...
            if left_peak and right_peak:
                if np.all(histogram[left_peak + 1:right_peak - 1] > 0) and (bottom_peak - left_peak < 25 or top_contour[max(0, left_peak - 7)] == 0) and (right_peak - bottom_peak < 30 or top_contour[min(width - 1, right_peak + 5)] == 0) and right_peak - left_peak > 20:
                    # white_pixels_between = self.calculate_white_pixels(skeleton, left_peak + 3, right_peak - 3, bottom_contour, top_contour)
                    ...
                    if bottom_contour[bottom_peak] + 75 > left_leg and bottom_contour[bottom_peak] + 75 > right_leg and (right_leg > bottom_contour[bottom_peak] + 15 and (left_leg > bottom_contour[bottom_peak] + 15 or np.any(histogram[max(0, left_peak - 10): left_peak + 2] > 10))):
                        if np.any(histogram[max(0, left_peak - 10): left_peak + 2] > 4) and (top_contour[current] > top_contour[bottom_peak] + 5 or top_contour[current] == 0 or top_contour[max(0, current - 5)] == 0 or top_contour[max(0, current - 5)] < top_contour[bottom_peak] + 5):
                            if self.count_white_pixels(skeleton, right_peak, bottom_contour, top_contour) < 5:
                                print(bottom_peak, 'n')
                                bottom_peaks = bottom_peaks[bottom_peaks != bottom_peak]
                                if right_peak not in bottom_peaks:
                                    bottom_peaks = np.concatenate((bottom_peaks, [right_peak]))
    ...
    def assign_tails_to_letter(self, word_image, peaks):
        ...
        for i in range(1, len(new_peaks) - 1):
            ...
            is_adjusted = False
            if np.all(top_contour[max(new_peaks[i] - 5, 0):new_peaks[i] + 1] != 0) and new_peaks[i] - new_peaks[i - 1] < 40 and top_contour[new_peaks[i]] + top_contour[new_peaks[i + 1]] != 0:
                if end - new_peaks[i] < 20:
                    if new_peaks[i + 1] + 5 > word_image.shape[1] - 1 or not np.all(top_contour[new_peaks[i]:new_peaks[i + 1] + 5] != 0) or bottom_contour[new_peaks[i + 1]] - top_contour[new_peaks[i + 1]] > 30:
                        ...
                        if len(letter_parts) < 2:
                            ...
                            is_adjusted = True
                if not is_adjusted:
                    current = min(new_peaks[i] + 5, word_image.shape[1] - 1)
                    # print(new_peaks[i], current, max(top_contour[max(new_peaks[i] - 3, 0): new_peaks[i] + 3]))
                    while current < word_image.shape[1] - 1 and ((bottom_contour[current] <= bottom_contour[current + 1] and top_contour[current] <= top_contour[current + 1] and top_contour[current] != 0) or top_contour[current] - 10 > min(top_contour[max(new_peaks[i] - 3, 0): new_peaks[i] + 3])):
                        current += 1
                    white_pixels_between = self.calculate_white_pixels(skeleton, max(new_peaks[i] - 5, 0), current, bottom_contour, top_contour)
                ...
                if not is_adjusted:
                    pixels_between = 0
                    for j in range(max(0, new_peaks[i] - 7), min(new_peaks[i] + 7, word_image.shape[1] - 1)):
                        pixels_between += 1 if self.count_white_pixels(skeleton, j,  bottom_contour, top_contour) > 0 else 0
                    if pixels_between > 5:
                        current = new_peaks[i]
                        while current < word_image.shape[1] - 1 and self.count_white_pixels(skeleton, current, bottom_contour, top_contour) > 0:
                            current += 1
                        letter_parts = self.prepare_letter_parts(new_word_image, new_peaks, i, current - new_peaks[i], False)
                        # print(new_peaks[i], current, letter_parts)
                        if len(letter_parts) < 2:
                            adjusted_peaks[i] = current + 5
                            is_adjusted = True
        return adjusted_peaks
    ...
    def assign_long_tails(self, word_image, peaks):
        ...
        for i in range(1, len(new_peaks) - 1):
            # print(lowest_line)
            skip_highest = False
            if min(new_peaks[i] + 7, new_peaks[i + 1]) - max(0, new_peaks[i] - 2) > 0:
                histogram = self.create_y_histogram(skeleton_image[lowest_line:, max(0, new_peaks[i] - 3):new_peaks[i + 1]])
                # print(histogram)
                if bottom_contour[new_peaks[i] - 1] != new_word_image.shape[0] - 1:
    ...
    def clean_ligature_segments(self, word_image, peaks):
        ...
        for i in range(len(peaks) - 1):
            if peaks[i] < peaks[i + 1]:
                is_ligature = True
                segment = skeleton_image[:, peaks[i]:peaks[i + 1]]
                height, width = segment.shape
                prev_column = 0
                for column in range(width - 1):
                    if bottom_contour[peaks[i] + column] - top_contour[peaks[i] + column] >= 3 and bottom_contour[peaks[i] + column] - top_contour[peaks[i] + column] != height - 1:
                        is_ligature = False
                    if abs(prev_column - top_contour[peaks[i] + column]) > 2 and prev_column != 0 and top_contour[peaks[i] + column] != 0:
                        is_ligature = False
                    prev_column = top_contour[peaks[i] + column]
                    if not is_ligature:
                        break
                if is_ligature:
    ...
    def character_segmentation(self, words, image):
        ...
        for word in words:
            ...
            new_bottom_peaks = self.merge_capital_letter(top_peaks, bottom_peaks, word_image)
            new_top_peaks, new_bottom_peaks = self.repair_oversegmentation_of_w_or_m(top_peaks, new_bottom_peaks, word_image, True)
            new_bottom_peaks, new_top_peaks = self.repair_oversegmentation_of_w_or_m(new_bottom_peaks, new_top_peaks, word_image, False)
            new_bottom_peaks = self.adjust_letter_e_or_c(word_image, new_bottom_peaks)
            peaks_to_consider = self.repair_oversegmentation_of_n(new_bottom_peaks, new_top_peaks, width, word_image)
            ...
            def show_img_and_contour(img, skeleton, new_image, peaks1, peaks2, peaks3):
                ...
                ax3.imshow(draw_contour_columns(new_image, peaks3), cmap='gray')
                plt.show()

            # show_img_and_contour(skeleton, skeleton, word_image, bottom_peaks, top_peaks, adjusted_peaks)
            # print("\n" * 100)
